...
pub fn get_sqrt_price_at_tick(tick: i32) -> Result<u128, anchor_lang::error::Error> {
    ...
    require!(abs_tick <= TICK_MAX as u32, ErrorCode::TickUpperOverflow);
...
pub fn get_tick_at_sqrt_price(sqrt_price_x64: u128) -> Result<i32, anchor_lang::error::Error> {
    ...
    require!(
        sqrt_price_x64 >= SQRT_PRICE_X64_MIN && sqrt_price_x64 < SQRT_PRICE_X64_MAX,
...
mod tick_math_test {
    ...
    mod get_sqrt_price_at_tick_test {
        ...
        fn check_get_sqrt_price_at_tick_at_min_or_TICK_MAX() {
            assert_eq!(
                get_sqrt_price_at_tick(TICK_MIN).unwrap(),
                SQRT_PRICE_X64_MIN
            );
            let min_sqrt_price = SQRT_PRICE_X64_MIN as f64 / fixed_point_64::Q64 as f64;
            println!("min_sqrt_price: {}", min_sqrt_price);
            assert_eq!(
                get_sqrt_price_at_tick(TICK_MAX).unwrap(),
                SQRT_PRICE_X64_MAX
            );
            let max_sqrt_price = SQRT_PRICE_X64_MAX as f64 / fixed_point_64::Q64 as f64;
    ...
    mod get_tick_at_sqrt_price_test {
        ...
        fn check_get_tick_at_sqrt_price_at_min_or_max_sqrt_price() {
            assert_eq!(
                get_tick_at_sqrt_price(SQRT_PRICE_X64_MIN).unwrap(),
                TICK_MIN,
            );

            // we can't reach SQRT_PRICE_X64_MAX
            assert_eq!(
                get_tick_at_sqrt_price(SQRT_PRICE_X64_MAX - 1).unwrap(),
                TICK_MAX - 1,
            );
        }
    }
    ...
    mod fuzz_tests {
        ...
        proptest! {
            ...
            fn get_sqrt_price_at_tick_test (
                tick in TICK_MIN+1..TICK_MAX-1,
            ) {
                let sqrt_price_x64 = get_sqrt_price_at_tick(tick).unwrap();

                assert!(sqrt_price_x64 >= SQRT_PRICE_X64_MIN);
                assert!(sqrt_price_x64 <= SQRT_PRICE_X64_MAX);
            ...
            fn get_tick_at_sqrt_price_test (
                sqrt_price in SQRT_PRICE_X64_MIN..SQRT_PRICE_X64_MAX
            ) {
                let tick = get_tick_at_sqrt_price(sqrt_price).unwrap();

                assert!(tick >= TICK_MIN);
                assert!(tick <= TICK_MAX);
            ...
            fn get_tick_at_sqrt_price_is_sequence_test (
                sqrt_price in (SQRT_PRICE_X64_MIN + 10)..SQRT_PRICE_X64_MAX
            ) {
